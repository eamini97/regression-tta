...
def get_svhn(train_transform, val_transform) -> tuple[datasets.SVHN, datasets.SVHN]:
    train_ds = datasets.SVHN(SVHN_PATH, split="train", download=True,
                             transform=train_transform,)
    val_ds = datasets.SVHN(SVHN_PATH, split="test", download=True, transform=val_transform)
    return train_ds, val_ds


def get_mnist(train_transform, val_transform) -> tuple[datasets.MNIST, datasets.MNIST]:
    train_ds = datasets.MNIST(MNIST_PATH, train=True, download=True,
                              transform=train_transform)
    val_ds = datasets.MNIST(MNIST_PATH, train=False, download=True, transform=val_transform)
    return train_ds, val_ds
